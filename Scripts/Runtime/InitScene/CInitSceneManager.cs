...
public abstract partial class CInitSceneManager : CSceneManager {
...
#if UNITY_EDITOR
	public override int ScriptOrder => KCDefine.SCRIPT_ORDER_INIT_SCENE_MANAGER;
#endif			// #if UNITY_EDITOR
	#endregion			// 프로퍼티
	...
	protected virtual void Setup() {
		// 테이블을 로드한다
		CValueTable.Instance.LoadValuesFromRes(KCDefine.TABLE_PATH_G_COMMON_VALUE_TABLE);
		CStringTable.Instance.LoadStringsFromRes(KCDefine.TABLE_PATH_G_COMMON_STRING_TABLE);
		...
		if(CAccess.IsDesktopPlatform()) {
			Screen.SetResolution(KCDefine.DESKTOP_WINDOW_WIDTH, 
				KCDefine.DESKTOP_WINDOW_HEIGHT, FullScreenMode.Windowed);
		} else {
		...
		CUserInfoStorage.Instance.LoadUserInfo(KCDefine.DATA_PATH_USER_INFO);
	...
	private IEnumerator OnStart() {
		if(!CSceneManager.IsInit) {
			...
			// iOS 플랫폼을 설정한다 {
#if UNITY_IOS
			Device.SetNoBackupFlag(KCDefine.DIR_PATH_WRITABLE);
			Device.SetNoBackupFlag(KCDefine.IMG_PATH_SCREENSHOT);
			...
			CBuildInfoTable.Create(KCDefine.SCRIPTABLE_PATH_G_BUILD_INFO_TABLE);
			CBuildOptionTable.Create(KCDefine.SCRIPTABLE_PATH_G_BUILD_OPTION_TABLE);
			CDefineSymbolTable.Create(KCDefine.SCRIPTABLE_PATH_G_DEFINE_SYMBOL_TABLE);
			CProjectInfoTable.Create(KCDefine.SCRIPTABLE_PATH_G_PROJECT_INFO_TABLE);
			CDeviceInfoTable.Create(KCDefine.SCRIPTABLE_PATH_G_DEVICE_INFO_TABLE);

#if ADS_ENABLE || TENJIN_ENABLE || FLURRY_ENABLE || FIREBASE_ENABLE
			CPluginInfoTable.Create(KCDefine.SCRIPTABLE_PATH_G_PLUGIN_INFO_TABLE);
#endif			// #if ADS_ENABLE || TENJIN_ENABLE || FLURRY_ENABLE || FIREBASE_ENABLE

#if PURCHASE_ENABLE
			CProductInfoTable.Create(KCDefine.SCRIPTABLE_PATH_G_PRODUCT_INFO_TABLE);
