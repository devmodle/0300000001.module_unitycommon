...
public static partial class CExtension {
	...
	//! 캔버스 로컬 위치를 반환한다
	public static Vector3 ExGetLocalPos(this PointerEventData a_oSender, GameObject a_oGameObject) {
		var stPos = a_oSender.ExGetWorldPos();
		return stPos.ExToLocal(a_oGameObject);
	}
	...
	//! 캔버스 로컬 비율 위치를 반환한다
	public static Vector3 ExGetLocalScalePos(this PointerEventData a_oSender, GameObject a_oGameObject, Vector3 a_stScale) {
	...
	//! 보정된 캔버스 로컬 위치를 반환한다
	public static Vector3 ExGetCorrectLocalPos(this PointerEventData a_oSender, GameObject a_oGameObject) {
	...
	public static Vector3 ExGetCorrectLocalScalePos(this PointerEventData a_oSender, GameObject a_oGameObject, Vector3 a_stScale) {
	...
	public static GameObject ExFindChild(this Scene a_stSender, string a_oName) {
		...
		for(int i = 0; i < oGameObjects?.Length; ++i) {
			var oGameObject = oGameObjects[i].ExFindChild(a_oName);
	...
	public static GameObject ExFindChild(this GameObject a_oSender, string a_oName, bool a_bIsIncludeSelf = true) {
		...
		foreach(var oGameObject in oEnumerator) {
			if(oGameObject.name.ExIsEquals(a_oName)) {
				return oGameObject;
			}
		}
	...
	public static List<GameObject> ExFindChildren(this Scene a_stSender, string a_oName) {
		...
		for(int i = 0; i < oGameObjects?.Length; ++i) {
			var oGameObjectList = oGameObjects[i].ExFindChildren(a_oName);

			if(oGameObjectList != null) {
				oTotalGameObjectList.AddRange(oGameObjectList);
			}
		}
	...
	public static List<GameObject> ExFindChildren(this GameObject a_oSender, string a_oName, bool a_bIsIncludeSelf = true) {
		...
		foreach(var oGameObject in oEnumerator) {
			if(oGameObject.name.ExIsEquals(a_oName)) {
				oGameObjectList.Add(oGameObject);
			}
		}
	...
	public static void ExSendMessage(this Scene a_stSender, string a_oName, string a_oMessage, object a_oParams) {
		var oGameObject = a_stSender.ExFindChild(a_oName);
	...
	public static void ExSendMessage(this GameObject a_oSender, string a_oName, string a_oMessage, object a_oParams) {
		var oGameObject = a_oSender.ExFindChild(a_oName);
		oGameObject?.SendMessage(a_oMessage, a_oParams, SendMessageOptions.DontRequireReceiver);
	}
	...
	public static void ExBroadcastMessage(this Scene a_stSender, string a_oMessage, object a_oParams) {
		...
		for(int i = 0; i < oGameObjects?.Length; ++i) {
			oGameObjects[i].ExBroadcastMessage(a_oMessage, a_oParams);
		}
	}
	...
	public static void ExSetEnableComponent<T>(this Scene a_stSender, string a_oName, bool a_bIsEnable) where T : Component {
		var oGameObject = a_stSender.ExFindChild(a_oName);
		oGameObject.ExSetEnableComponent<T>(a_bIsEnable);
	}
	...
	public static void ExSetEnableComponent<T>(this GameObject a_oSender, string a_oName, bool a_bIsEnable, bool a_bIsIncludeSelf = true) where T : Component {
		var oGameObject = a_oSender.ExFindChild(a_oName, a_bIsIncludeSelf);
		oGameObject.ExSetEnableComponent<T>(a_bIsEnable);
	}
	...
	public static T ExFindComponentInParent<T>(this GameObject a_oSender, string a_oName, bool a_bIsIncludeSelf = true) where T : Component {
		var oGameObject = a_oSender.ExFindParent(a_oName, a_bIsIncludeSelf);
		return oGameObject?.GetComponentInParent<T>();
	}
	...
	public static T[] ExFindComponentsInParent<T>(this GameObject a_oSender, string a_oName, bool a_bIsIncludeSelf = true) where T : Component {
		var oGameObject = a_oSender.ExFindParent(a_oName, a_bIsIncludeSelf);
		return oGameObject?.GetComponentsInParent<T>();
	}
