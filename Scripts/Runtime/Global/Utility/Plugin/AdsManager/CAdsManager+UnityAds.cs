...
public partial class CAdsManager : CSingleton<CAdsManager> {
	...
	public void OnLoadUnityAdsBannerAds() {
		CScheduleManager.Instance.AddCallback(KUDefine.KEY_ADS_M_UNITY_ADS_BANNER_ADS_LOAD_CALLBACK, () => {
			Func.ShowLog("CAdsManager.OnLoadUnityAdsBannerAds", KBDefine.LOG_COLOR_PLUGIN);
	...
	public void OnLoadFailUnityAdsBannerAds(string a_oMsg) {
		CScheduleManager.Instance.AddCallback(KUDefine.KEY_ADS_M_UNITY_ADS_BANNER_ADS_LOAD_FAIL_CALLBACK, () => {
			Func.ShowLog("CAdsManager.OnLoadFailUnityAdsBannerAds: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oMsg);
			m_stVariable.m_stUnityAdsVariable.m_nBannerAdsLoadTryTimes += 1;

			if(m_stVariable.m_stUnityAdsVariable.m_nBannerAdsLoadTryTimes < KUDefine.MAX_TIMES_ADS_LOAD_TRY) {
	...
	public void OnLoadUnityAds(string a_oPlacement) {
		CScheduleManager.Instance.AddCallback(KUDefine.KEY_ADS_M_UNITY_ADS_LOAD_CALLBACK, () => {
			Func.ShowLog("CAdsManager.OnLoadUnityAds: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oPlacement);
			...
			if(stResult.Key && stResult.Value.ExIsEquals(KUDefine.KEY_ADS_M_BANNER_ADS_PLACEMENT)) {
	...
	public void OnLoadFailUnityAds(string a_oMsg) {
		CScheduleManager.Instance.AddCallback(KUDefine.KEY_ADS_M_UNITY_ADS_LOAD_FAIL_CALLBACK, () => {
			Func.ShowLog("CAdsManager.OnLoadFailUnityAds: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oMsg);
			...
			for(int i = 0; i < m_stVariable.m_stUnityAdsVariable.m_oLoadingAdsPlacementList.Count; ++i) {
				...
				if(stResult.Key) {
					...
					if(stResult.Value.ExIsEquals(KUDefine.KEY_ADS_M_REWARD_ADS_PLACEMENT)) {
						m_stVariable.m_stUnityAdsVariable.m_nRewardAdsLoadTryTimes += 1;

						if(m_stVariable.m_stUnityAdsVariable.m_nRewardAdsLoadTryTimes < KUDefine.MAX_TIMES_ADS_LOAD_TRY) {
							this.LoadRewardAds(EAdsType.UNITY_ADS);
						}
					} else if(stResult.Value.ExIsEquals(KUDefine.KEY_ADS_M_FULLSCREEN_ADS_PLACEMENT)) {
						m_stVariable.m_stUnityAdsVariable.m_nFullscreenAdsLoadTryTimes += 1;

						if(m_stVariable.m_stUnityAdsVariable.m_nFullscreenAdsLoadTryTimes < KUDefine.MAX_TIMES_ADS_LOAD_TRY) {
	...
	public void OnCloseUnityAds(string a_oPlacement, ShowResult a_eShowResult) {
		CScheduleManager.Instance.AddCallback(KUDefine.KEY_ADS_M_UNITY_ADS_CLOSE_CALLBACK, () => {
			Func.ShowLog("CAdsManager.OnCloseUnityAds: {0}, {1}", KBDefine.LOG_COLOR_PLUGIN, a_oPlacement, a_eShowResult);
	...
	private bool IsLoadUnityAdsBannerAds() {
		CBAccess.Assert(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList.ContainsKey(KUDefine.KEY_ADS_M_BANNER_ADS_PLACEMENT));
		return Advertisement.IsReady(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_BANNER_ADS_PLACEMENT]);
	}

	//! 유니티 애즈 보상 광고 로드 여부를 검사한다
	private bool IsLoadUnityAdsRewardAds() {
		CBAccess.Assert(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList.ContainsKey(KUDefine.KEY_ADS_M_REWARD_ADS_PLACEMENT));
		return Advertisement.IsReady(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_REWARD_ADS_PLACEMENT]);
	}

	//! 유니티 애즈 전면 광고 로드 여부를 검사한다
	private bool IsLoadUnityAdsFullscreenAds() {
		CBAccess.Assert(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList.ContainsKey(KUDefine.KEY_ADS_M_FULLSCREEN_ADS_PLACEMENT));
		return Advertisement.IsReady(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_FULLSCREEN_ADS_PLACEMENT]);
	}
	...
	private void LoadUnityAdsBannerAds() {
		...
		string oPlacement = m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_BANNER_ADS_PLACEMENT];
		Advertisement.Banner.Load(oPlacement, oLoadOptions);
	}

	//! 유니티 애즈 보상 광고를 로드한다
	private void LoadUnityAdsRewardAds() {
		string oPlacement = m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_REWARD_ADS_PLACEMENT];
	...
	private void LoadUnityAdsFullscreenAds() {
		string oPlacement = m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_FULLSCREEN_ADS_PLACEMENT];
		m_stVariable.m_stUnityAdsVariable.m_oLoadingAdsPlacementList.ExAddValue(oPlacement);
	...
	private void ShowUnityAdsBannerAds() {
		if(m_stParameters.m_eBannerAdsType == EAdsType.UNITY_ADS) {
			Advertisement.Banner.Show(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_BANNER_ADS_PLACEMENT]);
		}
	}

	//! 유니티 애즈 보상 광고를 출력한다
	private void ShowUnityAdsRewardAds() {
		Advertisement.Show(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_REWARD_ADS_PLACEMENT]);
	}

	//! 유니티 애즈 전면 광고를 출력한다
	private void ShowUnityAdsFullscreenAds() {
		Advertisement.Show(m_stParameters.m_stUnityAdsParameters.m_oAdsPlacementList[KUDefine.KEY_ADS_M_FULLSCREEN_ADS_PLACEMENT]);
	}
