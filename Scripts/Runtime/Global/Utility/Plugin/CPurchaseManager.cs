...
public class CPurchaseManager : CSingleton<CPurchaseManager>, IStoreListener {
	...
	public void ConfirmPurchase(string a_oID, System.Action<CPurchaseManager, string, bool> a_oCallback) {
		CScheduleManager.Instance.AddCallback(KDefine.U_KEY_PURCHASE_M_CONFIRM_CALLBACK, () => {
	...
	private void HandlePurchaseResult(string a_oID, bool a_bIsSuccess, bool a_bIsInvokeCallback = true, bool a_bIsRemoveCallback = false) {
		CScheduleManager.Instance.AddCallback(KDefine.U_KEY_PURCHASE_M_PURCHASE_RESULT_CALLBACK, () => {
			Func.ShowLog("CPurchaseManager.HandlePurchaseResult: {0}, {1}, {2}, {3}", 
				KDefine.B_LOG_COLOR_PLUGIN, a_oID, a_bIsSuccess, a_bIsInvokeCallback, a_bIsRemoveCallback);
