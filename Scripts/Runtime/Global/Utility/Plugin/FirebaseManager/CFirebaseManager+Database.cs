...
public partial class CFirebaseManager : CSingleton<CFirebaseManager> {
	...
	public void SaveData(List<string> a_oNodeNameList, string a_oJSONString, System.Action<CFirebaseManager, bool> a_oCallback) {
		CBAccess.Assert(a_oJSONString.ExIsValid());
		Func.ShowLog("CFirebaseManager.SaveData: {0}, {1}", KBDefine.LOG_COLOR_PLUGIN, a_oNodeNameList, a_oJSONString);
		...
		if(!this.IsInit) {
			...
			Func.WaitAsyncTask(oDatabaseReference.SetRawJsonValueAsync(a_oJSONString), (a_oTask) => {
				Func.ShowLog("CFirebaseManager.OnSaveData: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oTask.Exception?.Message);
	...
	public void LoadData(List<string> a_oNodeNameList, System.Action<CFirebaseManager, string, bool> a_oCallback) {
		Func.ShowLog("CFirebaseManager.LoadData: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oNodeNameList);
		...
		if(!this.IsInit) {
			...
			Func.WaitAsyncTask(oDatabaseReference.GetValueAsync(), (a_oTask) => {
				Func.ShowLog("CFirebaseManager.OnLoadData: {0}", KBDefine.LOG_COLOR_PLUGIN, a_oTask.Exception?.Message);
