...
public class CSceneLoader : CSingleton<CSceneLoader> {
	...
	public void LoadScene(string a_oName,
		...
		float a_fDuration = KUDefine.DEF_DURATION_SCREEN_FADE_IN_ANI, LoadSceneMode a_eLoadSceneMode = LoadSceneMode.Single) {
		...
		if(!a_bIsAni) {
			...
			CBAccess.Assert(CSceneManager.RootSceneManager != null);
	...
	//! 추가 씬을 로드한다
	public void LoadAdditiveScene(string a_oName, bool a_bIsStartActivityIndicator = false) {
	...
	//! 추가 씬을 로드한다
	public void LoadAdditiveScenes(string[] a_oNames, bool a_bIsStartActivityIndicator = false) {
		CBAccess.Assert(a_oNames.ExIsValid());

		for(int i = 0; i < a_oNames.Length; ++i) {
			this.LoadAdditiveScene(a_oNames[i], a_bIsStartActivityIndicator);
		}
	}

	//! 추가 씬을 로드한다
	public void LoadAdditiveScenes(List<string> a_oNameList, bool a_bIsStartActivityIndicator = false) {
		CBAccess.Assert(a_oNameList.ExIsValid());

		for(int i = 0; i < a_oNameList.Count; ++i) {
			this.LoadAdditiveScene(a_oNameList[i], a_bIsStartActivityIndicator);
		}
	}
	...
	private void DoLoadScene(string a_oName,
		...
		if(!a_bIsUseLoadingScene) {
			...
			SceneManager.LoadScene(KBDefine.SCENE_NAME_LOADING, a_eLoadSceneMode);
		}
	}
	...
	private IEnumerator DoLoadSceneAsync(string a_oName,
		...
		yield return CBFactory.CreateWaitForSeconds(a_fDelay);
		...
		if(!a_bIsUseLoadingScene) {
			...
			yield return Func.WaitAsyncOperation(oAsyncOperation, (a_oAsyncOperation, a_bIsComplete) => {
				...
				if(!bIsActiveScene && a_oAsyncOperation.progress.ExIsGreateEquals(KUDefine.MAX_PERCENT_ASYNC_SCENE_LOAD)) {
					bIsActiveScene = true;
