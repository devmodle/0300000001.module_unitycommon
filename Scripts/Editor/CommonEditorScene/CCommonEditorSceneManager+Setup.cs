...
public static partial class CCommonEditorSceneManager {
	...
	private static void SetupScene() {
		...
		for(int i = KCDefine.B_INDEX_START; i < oLights.Length; ++i) {
			// 메인 광원 일 경우
		...
		for(int i = KCDefine.B_INDEX_START; i < oSceneManagers.Length; ++i) {
			// 태그 설정이 필요 할 경우
			if(!oSceneManagers[i].CompareTag(KCDefine.U_TAG_SCENE_MANAGER)) {
				oSceneManagers[i].tag = KCDefine.U_TAG_SCENE_MANAGER;
			}
			
			for(int j = KCDefine.B_INDEX_START; j < oCameras.Length; ++j) {
				// 에디터 카메라가 아닐 경우
		...
		if(oDebugConsoleList.ExIsValid()) {
			for(int i = KCDefine.B_INDEX_START; i < oDebugConsoleList.Count; ++i) {
				...
				if(oLogManager != null && oDebugLogItemList.ExIsValid()) {
					...
					oSerializeObj.ExSetPropertyValue(KCEditorDefine.B_PROPERTY_NAME_DEBUG_C_LOG_ITEM_PREFAB, (a_oProperty) => 
						a_oProperty.objectReferenceValue = oDebugLogItemList[KCDefine.B_INDEX_START]);
				}
			}
		}
		...
		if(oDebugLogItemList.ExIsValid()) {
			for(int i = KCDefine.B_INDEX_START; i < oDebugLogItemList.Count; ++i) {
		...
		if(oFPSCounterList.ExIsValid()) {
			for(int i = KCDefine.B_INDEX_START; i < oFPSCounterList.Count; ++i) {
	...
	private static void SetupLightOpts() {
			...
			stScene.name.ExIsEquals(oLightingSettings.name)) 
		{
			...
			if(oLightingSettingsAsset != null) {
				...
				for(int i = KCDefine.B_INDEX_START; i < oPropertyInfos.Length; ++i) {
