...
public static partial class CPlatformBuildOption {
	...
	public static void EditorInitialize() {
		// 테이블을 로드한다 {
		CPlatformBuildOption.BuildInfoTable = Resources.Load<CBuildInfoTable>(KUDefine.SCRIPTABLE_PATH_G_BUILD_INFO_TABLE);
		CPlatformBuildOption.BuildInfoTable?.Awake();

		CPlatformBuildOption.BuildOptionTable = Resources.Load<CBuildOptionTable>(KUDefine.SCRIPTABLE_PATH_G_BUILD_OPTION_TABLE);
		CPlatformBuildOption.BuildOptionTable?.Awake();

		CPlatformBuildOption.ProjectInfoTable = Resources.Load<CProjectInfoTable>(KUDefine.SCRIPTABLE_PATH_G_PROJECT_INFO_TABLE);
		CPlatformBuildOption.ProjectInfoTable?.Awake();

		CPlatformBuildOption.DefineSymbolTable = Resources.Load<CDefineSymbolTable>(KUDefine.SCRIPTABLE_PATH_G_DEFINE_SYMBOL_TABLE);
		...
		if(CPlatformBuildOption.DefineSymbolTable != null) {
			...
			foreach(var stKeyValue in CPlatformBuildOption.DefineSymbolListContainer) {
				CPlatformBuildOption.AddDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_IL2CPP_ENABLE);
				CPlatformBuildOption.AddDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_USE_CUSTOM_PROJECT_OPTION);
			}
		}
	}
