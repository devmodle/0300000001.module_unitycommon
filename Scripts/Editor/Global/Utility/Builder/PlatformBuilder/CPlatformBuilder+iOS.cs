...
public static partial class CPlatformBuilder {
	...
	public static void BuildiOSDevicePlatformDebug() {
		// 전처리기 심볼을 추가한다
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_LOGIC_TEST_ENABLE);
	...
	public static void BuildiOSDevicePlatformWithAutoPlayRelease() {
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_FPS_ENABLE);
	...
	public static void BuildiOSDevicePlatformAdhoc() {
		// 전처리기 심볼을 추가한다
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_ADHOC_BUILD);
	...
	public static void BuildiOSDevicePlatformWithRoboTestAdhoc() {
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_ROBO_TEST_ENABLE);
	...
	public static void BuildiOSDevicePlatformStore() {
		...
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.iOS, KDSEditorDefine.DEFINE_SYMBOL_STORE_BUILD);
	...
	public static void BuildiOSSimulatorPlatformDebug() {
		...
		CBEditorAccess.SetGraphicAPI(BuildTarget.iOS, KBEditorDefine.IOS_SIMULATOR_GRAPHICS_DEVICE_TYPES);
	...
	public static void RemoteBuildiOSPlatformDebug() {
		CPlatformBuilder.ExecuteiOSPlatformJenkinsBuild(KBDefine.BUILD_MODE_DEBUG,
			KBEditorDefine.JENKINS_DEBUG_BUILD_FUNC, KBEditorDefine.JENKINS_IOS_DEBUG_PIPELINE_NAME, CPlatformBuildOption.BuildInfoTable.iOSBuildInfo.m_oDevProfileID, KBEditorDefine.IOS_DEV_IPA_EXPORT_METHOD);
	}

	//! iOS 플랫폼을 원격 빌드한다
	[MenuItem("Utility/Build/Remote (Jenkins)/iOS/Release")]
	public static void RemoteBuildiOSPlatformRelease() {
		CPlatformBuilder.ExecuteiOSPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE,
			KBEditorDefine.JENKINS_RELEASE_BUILD_FUNC, KBEditorDefine.JENKINS_IOS_RELEASE_PIPELINE_NAME, CPlatformBuildOption.BuildInfoTable.iOSBuildInfo.m_oDevProfileID, KBEditorDefine.IOS_DEV_IPA_EXPORT_METHOD);
	}
	...
	public static void RemoteBuildiOSPlatformAdhoc() {
		CPlatformBuilder.ExecuteiOSPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE,
			KBEditorDefine.JENKINS_ADHOC_BUILD_FUNC, KBEditorDefine.JENKINS_IOS_ADHOC_PIPELINE_NAME, CPlatformBuildOption.BuildInfoTable.iOSBuildInfo.m_oAdhocProfileID, KBEditorDefine.IOS_ADHOC_IPA_EXPORT_METHOD);
	}
	...
	public static void RemoteBuildiOSPlatformStore() {
		CPlatformBuilder.ExecuteiOSPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE,
			KBEditorDefine.JENKINS_STORE_BUILD_FUNC, KBEditorDefine.JENKINS_IOS_STORE_PIPELINE_NAME, CPlatformBuildOption.BuildInfoTable.iOSBuildInfo.m_oStoreProfileID, KBEditorDefine.IOS_STORE_IPA_EXPORT_METHOD);
	}
	...
	private static void BuildiOSPlatform(BuildPlayerOptions a_oPlayerOptions) {
		...
		if(!Application.isBatchMode) {
			CBAccess.CopyDirectory(KBEditorDefine.IOS_SRC_PLUGIN_PATH, KBEditorDefine.IOS_DEST_PLUGIN_PATH, false);
		}

		// 빌드 옵션을 설정한다 {
		a_oPlayerOptions.target = BuildTarget.iOS;
		a_oPlayerOptions.targetGroup = BuildTargetGroup.iOS;
		a_oPlayerOptions.locationPathName = KBEditorDefine.IOS_BUILD_PATH;
		...
		// 빌드 디렉토리를 생성한다
		CBAccess.CreateDirectory(KBEditorDefine.IOS_ABSOLUTE_BUILD_PATH);
	...
	private static void OnPostProcessiOSBuild(BuildTarget a_eTarget, string a_oPath) {
#if UNITY_IOS
		string oPlistFilepath = string.Format(KBEditorDefine.IOS_INFO_PLIST_PATH_FORMAT, a_oPath);
		...
		if(oDocument.ExIsValid()) {
			oDocument.root.SetBoolean(KBEditorDefine.IOS_ENCRYPTION_ENABLE_KEY, false);
		...
		if(oProject != null) {
			...
			var oCapability = new ProjectCapabilityManager(oProjectFilepath,
				KBEditorDefine.PATH_CAPABILITY_ENTITLEMENTS_IOS, null, oGUID);
			...
			for(int i = 0; i < KAppDefine.G_EXTRA_CAPABILITY_TYPES_IOS.Length; ++i) {
				...
				oProject.AddCapability(oGUID, KAppDefine.G_EXTRA_CAPABILITY_TYPES_IOS[i], 
					KBEditorDefine.PATH_CAPABILITY_ENTITLEMENTS_IOS);
			}
