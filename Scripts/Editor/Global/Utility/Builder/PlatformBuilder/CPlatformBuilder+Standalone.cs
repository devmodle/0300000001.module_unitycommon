...
public static partial class CPlatformBuilder {
	...
	private static void BuildStandalonePlatformDebug(EStandalonePlatformType a_ePlatformType) {
		// 전처리기 심볼을 추가한다
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Standalone, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Standalone, KDSEditorDefine.DEFINE_SYMBOL_LOGIC_TEST_ENABLE);
	...
	private static void BuildStandalonePlatformWithAutoPlayRelease(EStandalonePlatformType a_ePlatformType) {
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Standalone, KDSEditorDefine.DEFINE_SYMBOL_FPS_ENABLE);
	...
	private static void BuildStandalonePlatform(BuildPlayerOptions a_oPlayerOptions, EStandalonePlatformType a_ePlatformType) {
		...
		if(a_ePlatformType == EStandalonePlatformType.WINDOWS) {
			...
			a_oPlayerOptions.locationPathName = KBEditorDefine.WINDOWS_BUILD_PATH;
		} else {
			a_oPlayerOptions.target = BuildTarget.StandaloneOSX;
			a_oPlayerOptions.locationPathName = KBEditorDefine.MAC_BUILD_PATH;
		}
		// 빌드 옵션을 설정한다 }

		// 빌드 디렉토리를 생성한다 {
		string oBuildPath = string.Format(KBEditorDefine.STANDALONE_ABSOLUTE_BUILD_PATH_FORMAT, 
			CBEditorAccess.GetStandalonePlatformName(a_ePlatformType));
	...
	private static void RemoteBuildStandalonePlatformDebug(EStandalonePlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteStandalonePlatformJenkinsBuild(KBDefine.BUILD_MODE_DEBUG, 
			KBEditorDefine.JENKINS_DEBUG_BUILD_FUNC, KBEditorDefine.JENKINS_STANDALONE_DEBUG_PIPELINE_NAME, a_ePlatformType);
	}

	//! 독립 플랫폼을 원격 빌드한다
	private static void RemoteBuildStandalonePlatformRelease(EStandalonePlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteStandalonePlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE, 
			KBEditorDefine.JENKINS_RELEASE_BUILD_FUNC, KBEditorDefine.JENKINS_STANDALONE_RELEASE_PIPELINE_NAME, a_ePlatformType);
	}
