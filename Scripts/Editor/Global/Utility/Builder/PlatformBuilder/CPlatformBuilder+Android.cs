...
public static partial class CPlatformBuilder {
	...
	private static void BuildAndroidPlatformDebug(EAndroidPlatformType a_ePlatformType) {
		// 전처리기 심볼을 추가한다
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_LOGIC_TEST_ENABLE);
	...
	private static void BuildAndroidPlatformWithAutoPlayRelease(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_FPS_ENABLE);
	...
	private static void BuildAndroidPlatformAdhoc(EAndroidPlatformType a_ePlatformType) {
		// 전처리기 심볼을 추가한다
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_ADHOC_BUILD);
	...
	private static void BuildAndroidPlatformWithRoboTestAdhoc(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_ROBO_TEST_ENABLE);
	...
	private static void BuildAndroidPlatformStore(EAndroidPlatformType a_ePlatformType) {
		...
		CPlatformBuildOption.AddDefineSymbol(BuildTargetGroup.Android, KDSEditorDefine.DEFINE_SYMBOL_STORE_BUILD);
	...
	private static void BuildAndroidPlatform(BuildPlayerOptions a_oPlayerOptions, EAndroidPlatformType a_ePlatformType) {
		...
		if(!Application.isBatchMode) {
			CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_PLUGIN_PATH, KBEditorDefine.ANDROID_DEST_PLUGIN_PATH, false);
		}

		// 빌드 옵션을 설정한다 {
		string oPlatform = CBEditorAccess.GetAndroidPlatformName(a_ePlatformType);
		string oFilename = string.Format(KBEditorDefine.ANDROID_BUILD_FILENAME_FORMAT, oPlatform);

		string oBuildFileExtension = EditorUserBuildSettings.buildAppBundle ? KBEditorDefine.ANDROID_AAB_BUILD_FILE_EXTENSION
			: KBEditorDefine.ANDROID_APK_BUILD_FILE_EXTENSION;

		a_oPlayerOptions.target = BuildTarget.Android;
		a_oPlayerOptions.targetGroup = BuildTargetGroup.Android;
		a_oPlayerOptions.locationPathName = string.Format(KBEditorDefine.ANDROID_BUILD_PATH_FORMAT, oPlatform, oFilename, oBuildFileExtension);
		...
		// 빌드 디렉토리를 생성한다
		string oBuildPath = string.Format(KBEditorDefine.ANDROID_ABSOLUTE_BUILD_PATH_FORMAT, oPlatform);
		CBAccess.CreateDirectory(oBuildPath);
	...
	private static void RemoteBuildAndroidPlatformDebug(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteAndroidPlatformJenkinsBuild(KBDefine.BUILD_MODE_DEBUG, 
			KBEditorDefine.JENKINS_DEBUG_BUILD_FUNC, KBEditorDefine.JENKINS_ANDROID_DEBUG_PIPELINE_NAME, KBEditorDefine.ANDROID_APK_BUILD_FILE_EXTENSION, a_ePlatformType);
	}

	//! 안드로이드 플랫폼을 원격 빌드한다
	private static void RemoteBuildAndroidPlatformRelease(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteAndroidPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE, 
			KBEditorDefine.JENKINS_RELEASE_BUILD_FUNC, KBEditorDefine.JENKINS_ANDROID_RELEASE_PIPELINE_NAME, KBEditorDefine.ANDROID_APK_BUILD_FILE_EXTENSION, a_ePlatformType);
	}

	//! 안드로이드 플랫폼을 원격 빌드한다
	private static void RemoteBuildAndroidPlatformAdhoc(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteAndroidPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE, 
			KBEditorDefine.JENKINS_ADHOC_BUILD_FUNC, KBEditorDefine.JENKINS_ANDROID_ADHOC_PIPELINE_NAME, KBEditorDefine.ANDROID_APK_BUILD_FILE_EXTENSION, a_ePlatformType);
	}

	//! 안드로이드 플랫폼을 원격 빌드한다
	private static void RemoteBuildAndroidPlatformStore(EAndroidPlatformType a_ePlatformType) {
		CPlatformBuilder.ExecuteAndroidPlatformJenkinsBuild(KBDefine.BUILD_MODE_RELEASE, 
			KBEditorDefine.JENKINS_STORE_BUILD_FUNC, KBEditorDefine.JENKINS_ANDROID_STORE_PIPELINE_NAME, KBEditorDefine.ANDROID_AAB_BUILD_FILE_EXTENSION, a_ePlatformType);
	}
