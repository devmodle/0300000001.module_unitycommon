...
public static partial class CPlatformBuildOption {
	...
	public static void SetupPluginProjects() {
		// iOS 플러그인을 복사한다
		CBAccess.CopyDirectory(KBEditorDefine.IOS_SRC_PLUGIN_PATH, KBEditorDefine.IOS_DEST_PLUGIN_PATH);

		// 안드로이드 플러그인을 복사한다 {
		CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_PLUGIN_PATH, KBEditorDefine.ANDROID_DEST_PLUGIN_PATH);
		CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_UNITY_PLUGIN_PATH, KBEditorDefine.ANDROID_DEST_UNITY_PLUGIN_PATH);

		CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_MANIFEST_PATH, KBEditorDefine.ANDROID_DEST_MANIFEST_PATH, false);
		CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_MAIN_TEMPLATE_PATH, KBEditorDefine.ANDROID_DEST_MAIN_TEMPLATE_PATH, false);
		CBAccess.CopyFile(KBEditorDefine.ANDROID_SRC_PROGUARD_PATH, KBEditorDefine.ANDROID_DEST_PROGUARD_PATH, false);
		// 안드로이드 플러그인을 복사한다 }
	...
	public static void SetupEditorOptions() {
		...
		EditorSettings.unityRemoteResolution = KBEditorDefine.EDITOR_OPTION_REMOTE_RESOLUTION;
		EditorSettings.unityRemoteCompression = KBEditorDefine.EDITOR_OPTION_REMOTE_COMPRESSION;
		EditorSettings.externalVersionControl = KBEditorDefine.EDITOR_OPTION_VERSION_CONTROL;
		EditorSettings.unityRemoteJoystickSource = KBEditorDefine.EDITOR_OPTION_JOYSTIC_SOURCE;
		EditorSettings.projectGenerationRootNamespace = string.Empty;
		EditorSettings.projectGenerationUserExtensions = KBEditorDefine.EDITOR_OPTION_EXTENSIONS;

		EditorSettings.prefabUIEnvironment = EditorFunc.FindAsset<SceneAsset>(KBEditorDefine.SCENE_NAME_PATTERN, new string[] {
			KBEditorDefine.DIR_PATH_AUTO_SCENES,
			KBEditorDefine.DIR_PATH_SCENES
		});

		EditorSettings.prefabRegularEnvironment = EditorFunc.FindAsset<SceneAsset>(KBEditorDefine.SCENE_NAME_PATTERN, new string[] {
			KBEditorDefine.DIR_PATH_AUTO_SCENES,
			KBEditorDefine.DIR_PATH_SCENES
		});
...
#if UNITY_IOS
		EditorSettings.unityRemoteDevice = KBEditorDefine.EDITOR_OPTION_IOS_REMOTE_DEVICE;
#elif UNITY_ANDROID
		EditorSettings.unityRemoteDevice = KBEditorDefine.EDITOR_OPTION_ANDROID_REMOTE_DEVICE;
#else
		EditorSettings.unityRemoteDevice = KBEditorDefine.EDITOR_OPTION_DISABLE_REMOTE_DEVICE;
#endif			// #if UNITY_IOS
		...
		// 사운드 옵션을 설정한다 {
		var oSndManager = EditorFunc.LoadAsset(KBEditorDefine.ASSET_PATH_SND_MANAGER);
		...
		if(oSndManager != null && CPlatformBuildOption.BuildOptionTable != null) {
			...
			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_SND_M_DISABLE_AUDIO, (a_oProperty) => {
				a_oProperty.boolValue = CPlatformBuildOption.BuildOptionTable.SndOption.m_bIsDisable;
			});

			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_SND_M_VIRTUALIZE_EFFECT, (a_oProperty) => {
				a_oProperty.boolValue = CPlatformBuildOption.BuildOptionTable.SndOption.m_bIsVirtualizeEffect;
			});

			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_SND_M_GLOBAL_VOLUME, (a_oProperty) => {
				a_oProperty.floatValue = CPlatformBuildOption.BuildOptionTable.SndOption.m_fGlobalVolume;
			});

			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_SND_M_ROLLOFF_SCALE, (a_oProperty) => {
				a_oProperty.floatValue = CPlatformBuildOption.BuildOptionTable.SndOption.m_fRolloffScale;
			});

			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_SND_M_DOPPLER_FACTOR, (a_oProperty) => {
		...
		var oTagManager = EditorFunc.LoadAsset(KBEditorDefine.ASSET_PATH_TAG_MANAGER);

		if(oTagManager != null) {
			var oSerializeObj = new SerializedObject(oTagManager);

#if !CUSTOM_TAG_ENABLE
			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_TAG_M_TAG, (a_oProperty) => {
#if !EXTRA_TAG_ENABLE
				a_oProperty.ClearArray();
#endif			// #if !EXTRA_TAG_ENABLE

				if(a_oProperty.arraySize < KUDefine.TAGS.Length) {
					for(int i = a_oProperty.arraySize; i < KUDefine.TAGS.Length; ++i) {
						a_oProperty.InsertArrayElementAtIndex(i);
					}
				}

				CBAccess.Assert(a_oProperty.arraySize >= KUDefine.TAGS.Length);

				for(int i = 0; i < a_oProperty.arraySize; ++i) {
					var oProperty = a_oProperty.GetArrayElementAtIndex(i);
					oProperty.stringValue = KUDefine.TAGS[i];
				}
			});
#endif			// #if !CUSTOM_TAG_ENABLE

#if !CUSTOM_SORTING_LAYER_ENABLE
			oSerializeObj.ExSetPropertyValue(KBEditorDefine.PROPERTY_NAME_TAG_M_SORTING_LAYER, (a_oProperty) => {
#if !EXTRA_SORTING_LAYER_ENABLE
				a_oProperty.ClearArray();
#endif			// #if !EXTRA_TAG_ENABLE

				if(a_oProperty.arraySize < KUDefine.SORTING_LAYERS.Length) {
					for(int i = a_oProperty.arraySize; i < KUDefine.SORTING_LAYERS.Length; ++i) {
						a_oProperty.InsertArrayElementAtIndex(i);
					}
				}

				CBAccess.Assert(a_oProperty.arraySize >= KUDefine.SORTING_LAYERS.Length);
				...
				for(int i = 0; i < a_oProperty.arraySize; ++i) {
					...
					oEnumerator.ExEnumerate((a_oValue) => {
						...
						string oSortingLayer = KUDefine.SORTING_LAYERS[i];

						if(oProperty.name.ExIsEquals(KBEditorDefine.PROPERTY_NAME_TAG_M_NAME)) {
							oProperty.stringValue = oSortingLayer;
						} else if(oProperty.name.ExIsEquals(KBEditorDefine.PROPERTY_NAME_TAG_M_UNIQUE_ID)) {
							oProperty.intValue = oSortingLayer.ExIsEquals(KUDefine.SORTING_LAYER_DEF) ? 0 : i + 10;
						}
					});
				}
			});
	...
	public static void SetupProjectOptions() {
		...
		string oSearch = KDSEditorDefine.DEFINE_SYMBOL_NOT_USE_CUSTOM_PROJECT_OPTION;
		string oReplace = KDSEditorDefine.DEFINE_SYMBOL_USE_CUSTOM_PROJECT_OPTION;

		for(int i = 0; i < KBEditorDefine.PATH_SCRIPT_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_SCRIPT_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, oSearch, oReplace, oEncoding, false);
		}
		// 스크립트를 복사한다 }

		// 데이터를 복사한다
		for(int i = 0; i < KBEditorDefine.PATH_DATA_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_DATA_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, false);
		}

		// 프리팹을 복사한다
		for(int i = 0; i < KBEditorDefine.PATH_PREFAB_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_PREFAB_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, false);
		}
		
		// 테이블을 복사한다
		for(int i = 0; i < KBEditorDefine.PATH_TABLE_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_TABLE_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, false);
		}

		// 스크립트 객체를 복사한다
		for(int i = 0; i < KBEditorDefine.PATH_SCRIPTABLE_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_SCRIPTABLE_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, false);
		}

		// 씬을 복사한다
		for(int i = 0; i < KBEditorDefine.PATH_SCENE_FILEPATH_INFOS.Length; ++i) {
			var stFilepathInfo = KBEditorDefine.PATH_SCENE_FILEPATH_INFOS[i];
			CBAccess.CopyFile(stFilepathInfo.Key, stFilepathInfo.Value, false);
		}
	...
	public static void SetupDefineSymbols() {
		...
		if(CPlatformBuildOption.DefineSymbolListContainer.ExIsValid()) {
			foreach(var stKeyValue in CPlatformBuildOption.DefineSymbolListContainer) {
				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_FPS_ENABLE);
				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_ADS_TEST_ENABLE);
				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_LOGIC_TEST_ENABLE);
				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_RECEIPT_CHECK_ENABLE);

				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_ADHOC_BUILD);
				CPlatformBuildOption.RemoveDefineSymbol(stKeyValue.Key, KDSEditorDefine.DEFINE_SYMBOL_STORE_BUILD);
			}
	...
	public static void SetupGraphicAPIs() {
		...
		CBEditorAccess.SetGraphicAPI(BuildTarget.StandaloneOSX, KBEditorDefine.MAC_GRAPHICS_DEVICE_TYPES);
		CBEditorAccess.SetGraphicAPI(BuildTarget.StandaloneWindows, KBEditorDefine.WINDOWS_GRAPHICS_DEVICE_TYPES);
		CBEditorAccess.SetGraphicAPI(BuildTarget.StandaloneWindows64, KBEditorDefine.WINDOWS_GRAPHICS_DEVICE_TYPES);

		// iOS 그래픽 API 를 설정한다
		PlayerSettings.iOS.sdkVersion = iOSSdkVersion.DeviceSDK;
		CBEditorAccess.SetGraphicAPI(BuildTarget.iOS, KBEditorDefine.IOS_DEVICE_GRAPHICS_DEVICE_TYPES);

		// 안드로이드 그래픽 API 를 설정한다
		CBEditorAccess.SetGraphicAPI(BuildTarget.Android, KBEditorDefine.ANDROID_GRAPHICS_DEVICE_TYPES);
	}
	...
	private static void SetupPlatformOptions() {
		...
		if(CPlatformBuildOption.ProjectInfoTable != null) {
			...
			CBAccess.Assert(bIsValidNumber);
	...
	private static void SetupStandalonePlatform() {
		if(CPlatformBuildOption.BuildInfoTable != null) {
			CExtension.ExSetPropertyValue<PlayerSettings.macOS>(null,
				KBEditorDefine.PROPERTY_NAME_CATEGORY, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildInfoTable.StandaloneBuildInfo.m_oCategory);
		}
	}
	...
	private static void SetupBuildOptions() {
		...
		PlayerSettings.SplashScreen.backgroundColor = KBEditorDefine.COLOR_UNITY_LOGO_BG;
		// 스플래시 옵션을 설정한다 }
		...
		// 리소스 압축 방식을 변경한다 {
		CExtension.ExCallFunc<EditorUserBuildSettings>(null, KBEditorDefine.FUNC_NAME_SET_COMPRESSION_TYPE, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, new object[] {
		...
		if(CPlatformBuildOption.BuildOptionTable != null) {
			...
			// 광원 맵 엔코딩 퀄리티를 설정한다 {
			CExtension.ExCallFunc<PlayerSettings>(null, KBEditorDefine.FUNC_NAME_SET_LIGHTMAP_ENCODING_QUALITY, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, new object[] {
			...
			// 광원 맵 스트리밍 여부를 설정한다 {
			CExtension.ExCallFunc<PlayerSettings>(null, KBEditorDefine.FUNC_NAME_SET_LIGHTMAP_STREAMING_ENABLE, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, new object[] {
			...
			CExtension.ExCallFunc<PlayerSettings>(null, KBEditorDefine.FUNC_NAME_SET_LIGHTMAP_STREAMING_PRIORITY, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, new object[] {
	...
	private static void SetupStandaloneBuildOptions() {
		...
		PlayerSettings.defaultScreenWidth = KBDefine.DESKTOP_WINDOW_WIDTH;
		PlayerSettings.defaultScreenHeight = KBDefine.DESKTOP_WINDOW_HEIGHT;
	...
	private static void SetupiOSBuildOptions() {
		...
		if(CPlatformBuildOption.BuildOptionTable != null) {
			...
			CExtension.ExSetPropertyValue<PlayerSettings.iOS>(null,
				KBEditorDefine.PROPERTY_NAME_APPLE_ENABLE_PRO_MOTION, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildOptionTable.iOSBuildOption.m_bIsEnableProMotion);

			CExtension.ExSetPropertyValue<PlayerSettings.iOS>(null,
				KBEditorDefine.PROPERTY_NAME_REQUIRE_AR_KIT_SUPPORT, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildOptionTable.iOSBuildOption.m_bIsRequreARKitSupport);

			CExtension.ExSetPropertyValue<PlayerSettings.iOS>(null,
				KBEditorDefine.PROPERTY_NAME_AUTO_ADD_CAPABILITIES, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildOptionTable.iOSBuildOption.m_bIsAutoAddCapabilities);
		}
	}
	...
	private static void SetupAndroidBuildOptions() {
		...
		CExtension.ExSetPropertyValue<PlayerSettings.Android>(null,
			KBEditorDefine.PROPERTY_NAME_VALIDATE_APP_BUNDLE_SIZE, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, true);
		...
		if(CPlatformBuildOption.BuildOptionTable != null) {
			...
			CExtension.ExSetPropertyValue<PlayerSettings.Android>(null,
				KBEditorDefine.PROPERTY_NAME_OPTIMIZE_FRAME_PACING, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildOptionTable.AndroidBuildOption.m_bIsOptimizeFramePacing);

			CExtension.ExSetPropertyValue<PlayerSettings.Android>(null,
				KBEditorDefine.PROPERTY_NAME_APP_BUNDLE_SIZE_TO_VALIDATE, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, CPlatformBuildOption.BuildOptionTable.AndroidBuildOption.m_nAppBundleSize);

			CExtension.ExSetPropertyValue<PlayerSettings.Android>(null,
				KBEditorDefine.PROPERTY_NAME_SUPPORTED_ASPECT_RATIO_MODE, KBDefine.BINDING_FLAG_NON_PUBLIC_STATIC, (int)CPlatformBuildOption.BuildOptionTable.AndroidBuildOption.m_eAspectRatioMode);
		}
	}
	#endregion			// 클래스 함수
}
