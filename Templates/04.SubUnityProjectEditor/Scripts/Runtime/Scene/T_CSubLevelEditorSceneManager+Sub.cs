...
#if (UNITY_STANDALONE && EXTRA_SCRIPT_MODULE_ENABLE) && (UTILITY_SCRIPT_TEMPLATES_MODULE_ENABLE && UTILITY_SCRIPT_TEMPLATES_MODULE_ENABLE)
				// 레벨 정보가 없을 경우
...
#endif			// #if (UNITY_STANDALONE && EXTRA_SCRIPT_MODULE_ENABLE) && (UTILITY_SCRIPT_TEMPLATES_MODULE_ENABLE && UTILITY_SCRIPT_TEMPLATES_MODULE_ENABLE)
...
#region 추가
		...
		private void ResetObjSprites() {
			...
			m_oGridInfoDict.ExReplaceVal(EKey.SEL_GRID_INFO, SampleEngineName.Factory.MakeGridInfo(m_oLevelInfoDict[EKey.SEL_LEVEL_INFO], Vector3.zero));
		...
		private void SetupObjSprites(STCellInfo a_stCellInfo, out Dictionary<EObjType, List<(EObjKinds, SpriteRenderer)>> a_oOutObjSpriteInfoDictContainer) {
			...
			foreach(var stKeyVal in a_stCellInfo.m_oObjKindsDictContainer) {
				...
				for(int i = 0; i < stKeyVal.Value.Count; ++i) {
					...
					oObjSprite.transform.localPosition = m_oGridInfoDict.GetValueOrDefault(EKey.SEL_GRID_INFO).m_stPivotPos + a_stCellInfo.m_stIdx.ExToPos(SampleEngineName.KDefine.E_OFFSET_CELL, SampleEngineName.KDefine.E_SIZE_CELL);
	...
	public partial class CSubLevelEditorSceneManager : CLevelEditorSceneManager, IEnhancedScrollerDelegate {
		...
		private void HandleTouchBeginEvent(CTouchDispatcher a_oSender, PointerEventData a_oEventData) {
			var stIdx = a_oEventData.ExGetLocalPos(this.ObjRoot).ExToIdx(m_oGridInfoDict.GetValueOrDefault(EKey.SEL_GRID_INFO).m_stPivotPos, SampleEngineName.KDefine.E_SIZE_CELL);
		}
