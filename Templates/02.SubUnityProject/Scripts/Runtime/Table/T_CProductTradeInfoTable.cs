...
public partial struct STProductTradeInfo {
	...
	public STProductTradeInfo(SimpleJSON.JSONNode a_oProductTradeInfo) {
		...
		for(int i = 0; i < KDefine.G_MAX_NUM_TARGET_INFOS; ++i) {
			...
			if(stTargetInfo.m_eTargetKinds.ExIsValid()) { m_oPayTargetInfoDict.TryAdd(Factory.MakeUniqueTargetInfoID(stTargetInfo.m_eTargetKinds, stTargetInfo.m_nKinds), stTargetInfo); }
		}
		...
		for(int i = 0; i < KDefine.G_MAX_NUM_TARGET_INFOS; ++i) {
			...
			if(stTargetInfo.m_eTargetKinds.ExIsValid()) { m_oAcquireTargetInfoDict.TryAdd(Factory.MakeUniqueTargetInfoID(stTargetInfo.m_eTargetKinds, stTargetInfo.m_nKinds), stTargetInfo); }
		}
	}
	#endregion			// 함수
}
