...
public partial class CObjInfoTable : CSingleton<CObjInfoTable> {
	...
	private void SetupJSONNodes(SimpleJSON.JSONNode a_oJSONNode, out List<SimpleJSON.JSONNode> a_oOutObjInfosList, out List<SimpleJSON.JSONNode> a_oOutObjEnhanceInfosList, out List<SimpleJSON.JSONNode> a_oOutBuyObjTradeInfosList, out List<SimpleJSON.JSONNode> a_oOutSaleObjTradeInfosList) {
		...
		var oTableInfoDictContainer = KDefine.G_TABLE_INFO_DICT_CONTAINER[Path.GetFileNameWithoutExtension(Access.ObjInfoTableLoadPath)];
		...
		for(int i = 0; i < oTableInfoDictContainer.Item2[this.GetType()][KCDefine.B_KEY_ENHANCE].Count; ++i) {
			a_oOutObjEnhanceInfosList.ExAddVal(a_oJSONNode[oTableInfoDictContainer.Item2[this.GetType()][KCDefine.B_KEY_ENHANCE][i]]);
		}
...
#else
		return this.DoLoadObjInfos(File.Exists(a_oFilePath) ? CFunc.ReadStr(a_oFilePath, true) : CFunc.ReadStrFromRes(a_oFilePath, true));
#endif			// #if UNITY_EDITOR || (UNITY_STANDALONE && (DEBUG || DEVELOPMENT_BUILD))
	}
