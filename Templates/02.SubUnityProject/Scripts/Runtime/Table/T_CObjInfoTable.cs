...
public partial struct STObjInfo {
	...
	public STObjInfo(SimpleJSON.JSONNode a_oObjInfo) {
		...
		for(int i = 0; i < KDefine.G_MAX_NUM_TARGET_INFOS; ++i) {
			...
			if(stTargetInfo.m_eTargetKinds.ExIsValid()) { m_oEquipItemTargetInfoDict.TryAdd(Factory.MakeUniqueTargetInfoID(stTargetInfo.m_eTargetKinds, stTargetInfo.m_nKinds), stTargetInfo); }
		}
