...
#if PURCHASE_MODULE_ENABLE
			stHandlerParams.m_oPurchaseCallbackDictA.TryAdd(CStoreUIsHandler.ECallback.PURCHASE, (a_oSender, a_oIDProduct, a_bIsSuccess) => {
				a_stParams.m_oPurchaseCallbackDictA?.GetValueOrDefault(ECallback.PURCHASE)?.Invoke(a_oSender, a_oIDProduct, a_bIsSuccess);
			});
